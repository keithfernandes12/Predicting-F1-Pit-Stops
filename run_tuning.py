"""
Optuna hyperparameter search: LGB (50 trials), XGB (30 trials), CatBoost (20 trials).
Uses 3-fold CV during search, then full 5-fold with best params.
Saves best params + tuned OOF predictions.
"""
...
print(f'Best LGB (3-fold): {lgb_study.best_value:.5f}  params={lgb_study.best_params}')
...
# ── XGBoost ─────────────────────────────────────────────────────────────────
print('\n=== XGBoost Optuna (30 trials, 3-fold) ===')
...
print(f'Best XGB (3-fold): {xgb_study.best_value:.5f}  params={xgb_study.best_params}')
...
# ── CatBoost ─────────────────────────────────────────────────────────────────
print('\n=== CatBoost Optuna (20 trials, 3-fold) ===')
...
print(f'Best CAT (3-fold): {cat_study.best_value:.5f}  params={cat_study.best_params}')

print('\n=== CAT best params → 5-fold CV ===')
cat_oof, cat_aucs = run_cv(train_feat, ext_feat, feature_cols, make_cat_fn(cat_study.best_params))
...
# ── Generate test predictions with tuned full models ─────────────────────────
...
# Estimate n_estimators as average of best iterations across folds (conservative)
n_est = 1500

print('\nTraining full tuned models for test predictions...')
...
with open(cache / 'best_params.pkl', 'wb') as f:
    pickle.dump(best_params, f)
with open(cache / 'oof_preds_tuned.pkl', 'wb') as f:
    pickle.dump(oof_tuned, f)
with open(cache / 'test_preds_tuned.pkl', 'wb') as f:
    pickle.dump(test_tuned, f)
